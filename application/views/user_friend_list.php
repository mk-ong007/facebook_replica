...
<style type="text/css">
	...
	@media (min-width: 576px) {
  	...
  	<div class=" container text-center text-light mt-4" id="mainDiv">
  		<h1 class="p-4">Friend List</h1>
  		<table class="table  text-light  table-hover">
  			<?php
                if(!empty( $this->session->flashdata('msg')))
                {
                    echo '<div class="text-center alert alert-success" >'.$this->session->flashdata('msg').'</div>';
                }
            ?>
  			 <tbody>
 	<?php 
			      	...
			      	<div class="row">
			      		<div class="p-2 dropdown">
							...
							<ul class="dropdown-menu" aria-labelledby="friends">
								<li><a style="width: 130px" href="<?php echo base_url('friend/unfriend/').$value->id ?>" class="dropdown-item" >Unfriend</a></li>
							</ul>
						</div>
						<div class="p-2">
 	...
 	if($noOfrequest == 0)
 	{
		echo "<tr><td colspan='3'>No friend found!</td></tr>";
 	}
 	 ?>
      ...
      if(toggleMessanger == false)
      {
        ...
        encode: true,
          ...
          $.each(obj, function(key,value) {
          	...
          	$('#msgReceived').html($('#msgReceived').html()+"<li class='fromHeader'><div class='dropdown-item' id="+value.friend_id+" data-bs-toggle='modal' data-bs-target='#messageOpen'><div class='d-flex '><span><img style='border-radius: 50%' id='' src="+ value.friend_dp +" alt='' width='42px' height='42px'></span><div class='px-2 text-start'><span style='text-decoration: none;' class='my-0 text-light ' >"+ value.friend_name +"<br><small>"+ value.message +"</small></span></div></div><hr></div></li>");
